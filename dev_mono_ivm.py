...
from dpvo.stream import image_stream, video_stream, image_stream_stereo, image_stream_stereo_ivm, image_stream_mono_ivm
...
def run(cfg, network, imagedir, calib, stride=0, skip=0, viz=False, timeit=False, save_reconstruction=False):
    ...
    reader = Process(target=image_stream_mono_ivm, args=(queue, imagedir, stride, skip))
    reader.start()
